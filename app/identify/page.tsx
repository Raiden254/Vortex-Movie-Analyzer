...
export default function IdentifyPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      {/* PAGE CONTENT */}
      <div style={{ maxWidth: "780px", margin: "0 auto", padding: "120px 24px 80px", position: "relative", zIndex: 1 }}>
        ...
        {/* TABS */}
        {status !== "done" && (
          <div className="tabs" style={{ display: "flex", background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: "10px", overflow: "hidden", marginBottom: "20px" }}>
            <button
              onClick={() => { setTab("image"); reset(); }}
              style={{ flex: 1, padding: "10px", fontSize: "13px", fontWeight: 500, fontFamily: "Syne, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", border: "none", cursor: "pointer", background: tab === "image" ? "var(--surface2)" : "transparent", color: tab === "image" ? "var(--blue)" : "var(--muted)", borderRight: "1px solid var(--border2)", transition: "all .15s" }}
            >
              📷 Screenshot
            </button>
            <button
              onClick={() => { setTab("video"); reset(); }}
              style={{ flex: 1, padding: "10px", fontSize: "13px", fontWeight: 500, fontFamily: "Syne, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", border: "none", cursor: "pointer", background: tab === "video" ? "var(--surface2)" : "transparent", color: tab === "video" ? "var(--blue)" : "var(--muted)", transition: "all .15s" }}
            >
              🎬 Video clip
            </button>
          </div>
        ...
        {status === "idle" && !file && (
          <div
            ...
            style={{ border: `1px dashed ${dragOver ? "var(--blue)" : "rgba(45,126,248,0.25)"}`, borderRadius: "16px", padding: "3rem 2rem", textAlign: "center", cursor: "pointer", background: dragOver ? "var(--blue-faint)" : "var(--surface)", transition: "all .2s" }}
          >
            <input
              ref={fileRef}
              type="file"
        ...
        {status === "analyzing" && (
          <div style={{ background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: "16px", padding: "48px 32px", textAlign: "center" }}>
            ...
            <div style={{ fontFamily: "DM Mono, monospace", fontSize: "11px", letterSpacing: "2px", textTransform: "uppercase", color: "var(--blue-bright)", marginBottom: "8px" }}>
              Ace is analyzing
            </div>
        ...
        {status === "done" && result && (
          <div style={{ display: "flex", flexDirection: "column", gap: "16px", animation: "fadeup 0.4s ease forwards" }}>
                ...
                <div style={{ flex: 1, minWidth: "220px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px", flexWrap: "wrap" }}>
                    ...
                    <div style={{ fontFamily: "DM Mono, monospace", fontSize: "10px", color: "var(--muted)" }}>
                      powered by Ace Analytics
                    </div>
                  </div>
                  <div style={{ fontSize: "clamp(24px,4vw,36px)", fontWeight: 800, letterSpacing: "-0.5px", color: "var(--text)", lineHeight: 1.05, marginBottom: "6px" }}>
                    {result.title}
                  </div>
                  <div style={{ fontFamily: "DM Mono, monospace", fontSize: "12px", color: "var(--muted)", marginBottom: "16px", lineHeight: 1.7 }}>
                    <span style={{ color: "var(--text)" }}>{result.year}</span> · {result.director} · {result.genre}<br />
                    {result.runtime} · ⭐ {result.rating}
                  </div>
                  <div style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.65, paddingTop: "16px", borderTop: "1px solid var(--border2)", marginBottom: "12px" }}>
                    {result.description}
                  </div>
                  <div style={{ fontFamily: "DM Mono, monospace", fontSize: "11px", color: "#EF9F27", fontStyle: "italic" }}>
                    {result.scene}
                  </div>
                </div>
              </div>
            ...
            {/* SIGNAL BREAKDOWN */}
            <div style={{ background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: "16px", padding: "24px" }}>
              <div style={{ fontFamily: "DM Mono, monospace", fontSize: "10px", letterSpacing: "2px", textTransform: "uppercase", color: "var(--blue-bright)", marginBottom: "16px" }}>
                Signal breakdown
              </div>
            ...
            <div style={{ textAlign: "center", paddingTop: "8px" }}>
              <button onClick={reset} style={{ padding: "12px 28px", borderRadius: "10px", border: "1px solid var(--border2)", background: "transparent", color: "var(--muted)", fontSize: "14px", cursor: "pointer", fontFamily: "Syne, sans-serif", fontWeight: 500, transition: "all .15s" }}>
